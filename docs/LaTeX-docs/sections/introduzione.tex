...
\textit{mespy} è una toolbox in Python che sto sviluppando per le esperienze di laboratorio del primo anno di fisica. L'obiettivo è fornire una base modulare e riutilizzabile per le operazioni ricorrenti nell'analisi dei dati sperimentali: statistica descrittiva e produzione di grafici.
...
Le librerie esterne utilizzate nel progetto sono \texttt{NumPy}, \texttt{pandas} e \texttt{Matplotlib}. Nel progetto \texttt{NumPy} e \texttt{pandas} vengono importati a livello di modulo con i seguenti acronimi standard:
\begin{minted}{python}
import numpy as np
import pandas as pd
\end{minted}

Quando serve generare figure, \texttt{matplotlib.pyplot} viene invece importato localmente all'interno delle funzioni grafiche:
\begin{minted}{python}
