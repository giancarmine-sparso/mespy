...
\section{\texorpdfstring{histogram(x, ddof, bins, label, xlabel, ylabel, title, bin\_ticks, bin\_width, tick\_rotation, decimals, figsize, save\_path, show\_legend, show\_mean, show\_std, dpi, ax, xlim, ylim, *, title\_fontsize, title\_pad, legend\_fontsize, legend\_loc, show\_grid, grid\_alpha, histo\_alpha, mean\_symbol, std\_alpha)}{histogram(x, ddof, bins, label, xlabel, ylabel, title, bin_ticks, bin_width, tick_rotation, decimals, figsize, save_path, show_legend, show_mean, show_std, dpi, ax, xlim, ylim, *, title_fontsize, title_pad, legend_fontsize, legend_loc, show_grid, grid_alpha, histo_alpha, mean_symbol, std_alpha)}}
...
\begin{apidesc}
  \item[\textbf{Parametri}]\hfill
    \begin{description}[font=\normalfont\ttfamily\bfseries, labelwidth=9em, leftmargin=9.5em, itemsep=1pt, parsep=0pt, topsep=3pt]
      ...
      \item[bin\_width]      \texttt{float} o \texttt{None} (default \texttt{None}) $\to$ larghezza fissa di ciascun bin; mutuamente esclusivo con \texttt{bins}
      ...
      \item[histo\_alpha]    float (default \texttt{0.85}) $\to$ trasparenza delle barre dell'istogramma
...
\texttt{np.floor} e \texttt{np.ceil} arrotondano il minimo e il massimo dei dati al multiplo di \texttt{bin\_width} più vicino (rispettivamente per difetto e per eccesso), garantendo che tutti i valori cadano all'interno degli intervalli generati da \texttt{np.arange}. Se \texttt{bin\_width} non è specificato, il parametro \texttt{bins} viene usato direttamente da \texttt{ax.hist()}.
...
\begin{apidesc}
  \item[\textbf{Parametri}]\hfill
    \begin{description}[font=\normalfont\ttfamily\bfseries, labelwidth=5em, leftmargin=5.5em, itemsep=1pt, parsep=0pt, topsep=3pt]
      ...
      \item[alpha]     \texttt{=histo\_alpha} $\to$ trasparenza configurabile delle barre
...
Subito dopo, la stringa di formato \texttt{fmt} viene costruita a partire dal parametro \texttt{decimals}. Se \texttt{bin\_ticks = True}, i tick dell'asse $x$ vengono posizionati sui bordi dei bin e formattati con il numero di decimali scelto. Il parametro \texttt{tick\_rotation} permette di ruotare le etichette per evitare sovrapposizioni quando i bin sono numerosi, mentre \texttt{histo\_alpha} consente di rendere le barre più o meno trasparenti senza modificare il resto dello stile.
\newpage
