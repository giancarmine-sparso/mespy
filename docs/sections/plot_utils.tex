...
\begin{itemize}
    \item \texttt{histogram(x, ...)}: genera un istogramma dei dati, con opzioni per titolo, legenda, griglia, trasparenza e indicatori statistici come media e banda di deviazione standard.
\end{itemize}
La documentazione per questa funzione è divisa in sottosezioni per spiegare al meglio ogni sua componente.
\section{\texorpdfstring{histogram(x, ddof, bins, label, xlabel, ylabel, title, bin\_ticks, bin\_width, tick\_rotation, decimals, figsize, save\_path, show\_legend, show\_mean, show\_std, dpi, ax, xlim, ylim, hist\_range, *, title\_fontsize, title\_pad, legend\_fontsize, legend\_loc, show\_grid, grid\_alpha, hist\_alpha, mean\_symbol, std\_alpha)}{histogram(x, ddof, bins, label, xlabel, ylabel, title, bin_ticks, bin_width, tick_rotation, decimals, figsize, save_path, show_legend, show_mean, show_std, dpi, ax, xlim, ylim, hist_range, *, title_fontsize, title_pad, legend_fontsize, legend_loc, show_grid, grid_alpha, hist_alpha, mean_symbol, std_alpha)}}
...
\begin{apidesc}
  \item[\textbf{Parametri}]\hfill
    \begin{description}[font=\normalfont\ttfamily\bfseries, labelwidth=9em, leftmargin=9.5em, itemsep=1pt, parsep=0pt, topsep=3pt]
      ...
      \item[bin\_width]      \texttt{float} o \texttt{None} (default \texttt{None}) $\to$ larghezza fissa di ciascun bin; usa \texttt{hist\_range} per definire l'estensione dei bordi ed \`e mutuamente esclusivo con \texttt{bins}
      ...
      \item[show\_grid]      \texttt{bool} (default \texttt{True}) $\to$ attiva la griglia orizzontale sull'asse $y$
      \item[grid\_alpha]     float (default \texttt{0.3}) $\to$ trasparenza della griglia
      \item[hist\_alpha]     float (default \texttt{0.85}) $\to$ trasparenza delle barre dell'istogramma
      \item[mean\_symbol]    stringa (default \texttt{r"\textbackslash bar\{x\}"}) $\to$ simbolo LaTeX usato nella legenda per la media
      \item[show\_std]       \texttt{bool} (default \texttt{True}) $\to$ mostra la banda $\pm\sigma$
      \item[std\_alpha]      float (default \texttt{0.15}) $\to$ trasparenza della banda $\pm\sigma$
      ...
      \item[hist\_range]     tupla o \texttt{None} $\to$ intervallo \texttt{(xmin, xmax)} usato per costruire i bordi dei bin quando \texttt{bin\_width} \`e specificato
    \end{description}
    I parametri da \texttt{title\_fontsize} a \texttt{std\_alpha} sono opzionali e keyword-only, cos\`i l'ordine posizionale storico della funzione resta invariato.
  \item[\textbf{Vincoli}]
  ...
  \item[\textbf{Errori}]
    ...
    \texttt{ValueError} se \texttt{hist\_range} non ha esattamente 2 elementi o se non soddisfa \texttt{xmin < xmax};
    \texttt{ValueError} se \texttt{bin\_width} $\leq 0$;
    \texttt{ValueError} se \texttt{bin\_width} e \texttt{bins} sono entrambi specificati.
\end{apidesc}
...
Il primo passo converte l'input in un array \texttt{float64}. La gestione dell'asse permette di integrare la funzione sia in figure autonome sia in griglie di sottografici: se \texttt{ax = None} viene creata una nuova figura, importando \texttt{matplotlib.pyplot} solo quando serve; altrimenti si usa l'asse fornito e si recupera la figura padre con \texttt{ax.get\_figure()}. Il parametro \texttt{ax} sta per \textit{axes} in inglese e fa parte della libreria \texttt{matplotlib}. Il parametro \texttt{ddof} controlla i gradi di libertà usati nel calcolo della deviazione standard (divisore $N - \text{ddof}$), mentre \texttt{title\_fontsize} e \texttt{title\_pad} permettono di regolare in modo fine l'aspetto del titolo. Se si vuole imporre un'estensione esplicita ai bordi dei bin costruiti con \texttt{bin\_width}, si pu\`o usare \texttt{hist\_range = (xmin, xmax)}.
\newpage
\subsection{Binnaggio e istogramma}
Se viene specificato \texttt{hist\_range}, la funzione valida l'intervallo \texttt{(xmin, xmax)} da usare per costruire i bin; in caso contrario usa direttamente il minimo e il massimo dei dati. Quando \texttt{bin\_width} \`e valorizzato, i bordi dei bin vengono poi allineati a multipli della larghezza richiesta:
\begin{minted}{python}
    # --- binnaggio ---
...
\texttt{np.floor} e \texttt{np.ceil} arrotondano \texttt{xmin} e \texttt{xmax} al multiplo di \texttt{bin\_width} pi\`u vicino (rispettivamente per difetto e per eccesso), garantendo che tutti i valori nell'intervallo scelto cadano all'interno degli intervalli generati da \texttt{np.arange}. Se \texttt{hist\_range} non \`e fornito, \texttt{xmin} e \texttt{xmax} coincidono con il minimo e il massimo dei dati; se invece \texttt{bin\_width} non \`e specificato, il parametro \texttt{bins} viene usato direttamente da \texttt{ax.hist()}.
...
\begin{apidesc}
  \item[\textbf{Parametri}]\hfill
    \begin{description}[font=\normalfont\ttfamily\bfseries, labelwidth=5em, leftmargin=5.5em, itemsep=1pt, parsep=0pt, topsep=3pt]
      ...
      \item[alpha]     \texttt{=hist\_alpha} $\to$ trasparenza configurabile delle barre
...
Subito dopo, la stringa di formato \texttt{fmt} viene costruita a partire dal parametro \texttt{decimals}. Se \texttt{bin\_ticks = True}, i tick dell'asse $x$ vengono posizionati sui bordi dei bin e formattati con il numero di decimali scelto. Il parametro \texttt{tick\_rotation} permette di ruotare le etichette per evitare sovrapposizioni quando i bin sono numerosi, mentre \texttt{hist\_alpha} consente di rendere le barre più o meno trasparenti senza modificare il resto dello stile.
\newpage
...
Il parametro \texttt{mean\_symbol} permette inoltre di personalizzare il simbolo mostrato in legenda, purché espresso come contenuto valido in modalità matematica LaTeX.
...
l'intervallo di ampiezza pari a una deviazione standard attorno al valore centrale. Questa banda fornisce una rappresentazione visiva immediata della dispersione dei dati intorno alla media. Il parametro \texttt{std\_alpha} ne controlla la trasparenza, permettendo di bilanciare meglio la leggibilità della banda rispetto alle barre dell'istogramma.
...
\begin{minted}{python}
    ...
    if ylim is not None:
        ax.set_ylim(ylim)
\end{minted}
L'etichetta dell'asse \(x\) è assegnata tramite il parametro \texttt{xlabel}, mentre per l'asse \(y\) viene utilizzata l'etichetta fornita in \texttt{ylabel}; se questa non è specificata, viene usato il valore predefinito \texttt{"Conteggi"}. Il titolo del grafico è fissato da \texttt{title} e può essere rifinito tramite \texttt{title\_fontsize} e \texttt{title\_pad}. La chiamata \texttt{ax.grid(...)} aggiunge una griglia leggera lungo l'asse dei conteggi solo se \texttt{show\_grid = True}, con opacità controllata da \texttt{grid\_alpha}. Se \texttt{show\_legend = True}, \texttt{ax.legend()} aggiunge una legenda che raccoglie e descrive gli elementi grafici opzionali, usando \texttt{legend\_fontsize} e \texttt{legend\_loc} per dimensione e posizione. I parametri \texttt{xlim} e \texttt{ylim} permettono di fissare manualmente i limiti degli assi; se non specificati, vengono determinati automaticamente da matplotlib. In questo punto della funzione i limiti sono solo applicati con \texttt{ax.set\_xlim(...)} e \texttt{ax.set\_ylim(...)}: la validazione della sequenza passata avviene invece all'inizio della funzione tramite \texttt{\_validate\_axis\_limits(...)}.
