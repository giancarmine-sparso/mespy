...
\section{\texorpdfstring{lin\_fit(x, y, sigma\_y, xlabel, ylabel, dpi, band, plot)}{lin\_fit(x, y, sigma\_y, xlabel, ylabel, dpi, band, plot)}}
...
\[
\text{Var}[\hat{m}] = \frac{1}{\text{Var}[x] \sum_i \frac{1}{\sigma^2_{Y_i}}}, \qquad \text{Var}[\hat{c}] = \frac{\overline{x^2}}{\text{Var}[x] \sum_i \frac{1}{\sigma_{Y_i}}} = \overline{x^2} \cdot \text{Var}[\hat{m}]
\]
e per la covarianza:
\[
...
\[
\sigma_r = \sqrt{\frac{\sum_i (y_i - (m \cdot x_i + c)^2)}{n - 2}}
\]
...
\begin{minted}{python}
    ...
    if plot:
        ...
        fig, (ax_fit, ax_res) = plt.subplots(
            ...
            figsize=(8, 6),
            dpi=dpi,
            gridspec_kw={"height_rations": [3, 1]},
            sharex=True,
