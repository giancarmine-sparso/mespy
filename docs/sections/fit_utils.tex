...
\section{\texorpdfstring{lin\_fit(x, y, sigma\_y, sigma\_x, xlabel, ylabel, dpi, band, plot, xlim, ylim, decimals, legend, legend\_coefficient, tol)}{lin\_fit(x, y, sigma\_y, sigma\_x, xlabel, ylabel, dpi, band, plot, xlim, ylim, decimals, legend, legend\_coefficient, tol)}}
...
La chiamata \texttt{ax\_res.errorbar(...)} riusa la stessa logica già descritta per \texttt{ax\_fit.errorbar}, compreso il parametro \texttt{xerr} per le barre d'errore orizzontali: cambia però la grandezza rappresentata sull'asse verticale, perché al posto dei valori \texttt{y} vengono usati i residui \texttt{r}. In questo modo il pannello inferiore non mostra più i dati originali, ma quanto ciascun punto si discosta dalla retta di fit.
