...
\begin{description}
  \item[\textbf{Parametri}]
    ...
    \texttt{label}: stringa (default: \texttt{"Dati"}) $\to$ testo che compare nella legenda;\\
...
\begin{description}
    \item[Parametri] 
    \texttt{bins=bins}: quanti intervalli usare, deciso dall'utente, default "auto" (scelti automaticamente da numpy);\\
    \texttt{density=False}: asse y a conteggi;\\
    \texttt{color="\#4878CF"}: colore delle barre;\\
    \texttt{edgecolo="white"}: bordo bianco tra una barra e l'altra;\\
    \texttt{alpha=0.85}: leggera trasparenza applicata alle barre, per distinguerle meglio;\\
    \texttt{label=label}: testo che compare nella legenda, controllato dal parametro \texttt{label} (default: \texttt{"Dati"});\\
\end{description}
\newpage
...
Successivamente abbiamo la linea mediana:
\begin{minted}{python}
    # --- linea media ---
    if show_mean:
        ax.axvline(
